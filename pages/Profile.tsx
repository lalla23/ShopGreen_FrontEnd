...
import { User, Lock, Save, AlertCircle, CheckCircle, Mail, ArrowRight, RefreshCw, ShoppingBag, Plus, Trash2, Link as LinkIcon, Edit3, Check } from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ userRole, currentUserName, onUpdateProfile, sellers, onSaveSellerProfile }) => {
  const location = useLocation();
  const [viewState, setViewState] = useState<ViewState>('PROFILE_EDIT');
  const [activeTab, setActiveTab] = useState<'personal' | 'seller'>('personal');
  ...
  const [sellerLinks, setSellerLinks] = useState<{ name: string; url: string }[]>([{ name: '', url: '' }]);
  ...
  useEffect(() => {
    if (currentUserName) {
      ...
      const existingSeller = sellers.find(s => s.username === currentUserName);
      if (existingSeller) {
        ...
        setSellerLinks(existingSeller.platformLinks.length > 0 ? existingSeller.platformLinks : [{ name: '', url: '' }]);
      }
    }
  ...
  const handleSaveUsername = (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim()) {
      setMessage({ type: 'error', text: 'Lo username non può essere vuoto.' });
      return;
    }
  ...
  const handleSaveNewPassword = (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setMessage({ type: 'error', text: 'Password non valida. Inserisci almeno 6 caratteri.' });
      return;
    }
  ...
  const handleSaveSeller = (e: React.FormEvent) => {
    ...
    if (sellerZones.length === 0) {
      setMessage({ type: 'error', text: 'Seleziona almeno una zona di scambio.' });
      return;
    }

    // Clean data
    const cleanedTags = sellerTags.split(',').map(t => t.trim()).filter(t => t.length > 0);
    const cleanedLinks = sellerLinks.filter(l => l.name.trim() && l.url.trim());

    if (cleanedTags.length === 0) {
      setMessage({ type: 'error', text: 'Inserisci almeno una categoria (es. Libri, Vestiti).' });
      return;
    }

    const newSellerProfile: Seller = {
      id: isSellerActive ? (sellers.find(s => s.username === currentUserName)?.id || 'temp') : `s_${Date.now()}`,
      username: currentUserName || 'Unknown',
      zoneIds: sellerZones,
      bio: sellerBio,
      categories: cleanedTags,
      platformLinks: cleanedLinks,
      avatarUrl: 'https://images.unsplash.com/photo-1511367461989-f85a21fda167?auto=format&fit=crop&q=80&w=200' // Default placeholder for now
    };

    onSaveSellerProfile(newSellerProfile);
    setIsSellerActive(true);
    setMessage({ type: 'success', text: 'Card E-commerce salvata con successo!' });
  };
    ...
    <div className="animate-in fade-in duration-300">
       <div className="bg-green-50 p-4 rounded-xl border border-green-100 mb-6">
         <h3 className="text-green-800 font-bold mb-1 flex items-center gap-2">
            <ShoppingBag className="w-5 h-5"/> Card E-commerce
         </h3>
         <p className="text-sm text-green-700">
           Crea o modifica la scheda visibile agli altri utenti nella sezione E-commerce. Qui puoi elencare cosa vendi e come contattarti.
         </p>
       </div>
       ...
       <form onSubmit={handleSaveSeller} className="space-y-6">
          ...
          <div className="space-y-2">
             <label className="text-sm font-semibold text-gray-700">Zone di scambio (seleziona multiple)</label>
             ...
             {sellerZones.length === 0 && <p className="text-xs text-red-500 mt-1">Seleziona almeno una zona</p>}
          </div>

          <div className="space-y-2">
             <label className="text-sm font-semibold text-gray-700">Bio / Informazioni</label>
             <textarea 
               rows={3}
               value={sellerBio}
          ...
          <div className="space-y-2">
             <label className="text-sm font-semibold text-gray-700">Categorie (separate da virgola)</label>
             <input 
               type="text" 
               value={sellerTags}
             ...
             />
             <p className="text-xs text-gray-500">Questi tag aiuteranno gli altri a trovarti nei filtri.</p>
          </div>

          <div className="space-y-3">
             <label className="text-sm font-semibold text-gray-700 flex justify-between items-center">
               <span>Link Piattaforme Esterne</span>
               <button type="button" onClick={addLinkField} className="text-xs text-green-600 font-bold flex items-center gap-1 hover:underline">
          ...
          <div className="pt-4">
             ...
             >
                ...
                {isSellerActive ? 'Aggiorna Card' : 'Crea Card Venditore'}
             </button>
          </div>
  ...
  const renderProfileEdit = () => (
    <div className="space-y-8 animate-in fade-in duration-300">
        <form onSubmit={handleSaveUsername} className="space-y-4">
            <h2 className="text-lg font-bold text-gray-800 border-b border-gray-100 pb-2">Dati Personali</h2>
            <div className="space-y-2">
        ...
        <div className="space-y-4 pt-2">
            <h2 className="text-lg font-bold text-gray-800 border-b border-gray-100 pb-2">Sicurezza</h2>
            <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                ...
                <p className="text-xs text-gray-500 italic">
                    Clicca sul campo password per richiedere la modifica. Ti invieremo una email.
                </p>
            </div>
        </div>
    </div>
  ...
  const renderEmailSent = () => (
      <div className="text-center py-10 space-y-6 animate-in zoom-in duration-300">
          ...
          <div>
              <h2 className="text-2xl font-bold text-gray-900">Controlla la tua posta</h2>
              <p className="text-gray-600 mt-2 max-w-md mx-auto">
                  Abbiamo inviato una email all'indirizzo associato al tuo account. Clicca sul link all'interno per reimpostare la password.
              </p>
          </div>
          <div className="pt-6 border-t border-gray-100 mt-6">
              <p className="text-xs text-gray-400 font-mono mb-2">[SIMULAZIONE SVILUPPO]</p>
              <button 
          ...
          <button onClick={() => setViewState('PROFILE_EDIT')} className="text-sm text-gray-500 underline mt-4">Torna indietro</button>
      </div>
  );
  ...
  const renderResetForm = () => (
      <div className="space-y-6 animate-in slide-in-from-right duration-300">
           <div className="flex items-center gap-2 mb-4 text-green-700 bg-green-50 p-3 rounded-lg border border-green-100">
              ...
              <span className="font-semibold text-sm">Reimpostazione Password in corso...</span>
           </div>
           ...
           <form onSubmit={handleSaveNewPassword} className="space-y-5">
              ...
              <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Reinserisci Password</label>
                  <input 
                    type="password" 
    ...
    <div className="max-w-3xl mx-auto p-4 md:p-8">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="bg-green-50 p-6 border-b border-green-100 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
                {viewState === 'RESET_PASSWORD_FORM' ? 'Reimposta Password' : 'Il tuo Account'}
            </h1>
            <p className="text-green-700 font-medium flex items-center gap-2">
               {userRole === UserRole.OPERATOR ? 'Operatore Autorizzato' : 'Utente Standard'}
               {isSellerActive && <span className="bg-green-600 text-white text-[10px] px-2 py-0.5 rounded-full uppercase">Venditore Attivo</span>}
            </p>
          </div>
        </div>
        ...
        {viewState === 'PROFILE_EDIT' && (
          <div className="flex border-b border-gray-100">
            <button 
              ...
              className={`flex-1 py-4 text-center font-bold text-sm transition-colors relative ${activeTab === 'personal' ? 'text-green-700 bg-green-50/50' : 'text-gray-500 hover:bg-gray-50'}`}
            >
              Dati Personali
              {activeTab === 'personal' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-green-600"></div>}
            </button>
            <button 
              onClick={() => setActiveTab('seller')}
              className={`flex-1 py-4 text-center font-bold text-sm transition-colors relative ${activeTab === 'seller' ? 'text-green-700 bg-green-50/50' : 'text-gray-500 hover:bg-gray-50'}`}
            >
              La tua Card E-commerce
              {activeTab === 'seller' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-green-600"></div>}
            </button>
          </div>
        )}
        ...
        <div className="p-6 md:p-8">
          ...
          {viewState === 'PROFILE_EDIT' && (
             activeTab === 'personal' ? renderProfileEdit() : renderSellerTab()
          )}
